    ...
    xhr.addEventListener('load', () => {
        if (xhr.status >= 200 && xhr.status < 300) {
            try {
                ...
                const tickets = Array.from(ticketContainer.querySelectorAll('.ticket-card'));

                tickets.forEach(item => {
                    item.addEventListener('click', (e) => {
                        if (e.target.classList.contains('ticket-content')){
                            item.querySelector('.ticket-hidden').classList.toggle('ticket-visible')
                        }
                    })

                    item.querySelector('.delete-button').addEventListener('click', () => {
                        xhr.open('POST', `http://localhost:8080/?method=deleteTicket&id=${item.dataset.id}`);
                        xhr.send()
                    ...
                    item.querySelector('.edit-button').addEventListener('click', () => {
                        ...
                        descInpt.value = item.querySelector('.ticket-description').textContent
                        ...
                        submitBtn.addEventListener('click', () => {
                            ...
                            xhr.open('POST', `http://localhost:8080/?method=editTicket&id=${item.dataset.id}`);
                            xhr.send(form)
                    ...
                    item.querySelector('.ticket-status').addEventListener('click', () => {
                        xhr.open('POST', `http://localhost:8080/?method=ticketDone&id=${item.dataset.id}`);
                        xhr.send()
